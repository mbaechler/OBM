...
public class SequenceInputStream extends InputStream {
	 ...
	 * 
	 * @param e a list of InputStreams to concat
	 */
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		while( read != len && index < streams.size()-1) {
			if (read == -1) read = 0;
			index++;
			int nextread = ((InputStream)streams.get(index)).read(b,off+read,len-read);
			
			if( nextread != -1) read += nextread;
		}
	...
	public int available() throws IOException {
		...
		for( int i=0; i<streams.size(); i++) {
			size += ((InputStream)streams.get(i)).available();
		}
		return size;
	}

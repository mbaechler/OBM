...
public class FileSource implements Source {
	...
	/**
	 * Constructs the FileSource. If the parameter isTemp is 
	 * true, then the file will be erased once all references
	 * are closed.
	 * 
	 * 
	 * @param file
	 * @throws IOException
	 */
	public FileSource( File file, boolean isTemp ) throws IOException {
		model = new FileSourceModel( file, isTemp );
		model.incReferences();
		start = 0;
		end = model.length();
		pos = 0;

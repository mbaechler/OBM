...
public class SendEmailHandler implements ContentHandler {
	...
	private Address[] parseHeader(String mailboxList) throws ParserException {
		org.columba.ristretto.message.Address[] list = AddressParser.parseMailboxList(mailboxList);
		ArrayList<Address> addresses = new ArrayList<Address>();
		for (org.columba.ristretto.message.Address addr: list) {
			addresses.add(new Address(addr.getMailAddress()));
		}
	...
	public void field(Field field) throws MimeException {
		if (!inBody) {
			if ("to".equalsIgnoreCase(field.getName())) {
				try {
					Address[] adds = parseHeader(field.getBody());

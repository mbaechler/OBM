...
public class Event implements Indexed<Integer> {
	...
	public void setDuration(int duration) {
		Preconditions.checkArgument(duration >= 0, "Duraction must be a positive integer value");
	...
	public Event clone() {
		...
		event.alert = alert;
		event.allday = allday;
		event.attendees = new LinkedList<Attendee>(attendees);
	...
	public Event getEventInstanceWithRecurrenceId(Date recurrenceId){
		...
		if(instance == null){
			instance = clone();
			instance.startDate = recurrenceId;
